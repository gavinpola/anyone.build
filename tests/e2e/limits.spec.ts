...
/** No per-person daily cap: a guest can ask more than once a day. (Gavin: "no limit".) */
...
async function ask(page: Page, text: string) {
  ...
  // any real verdict is fine; the one thing that must NOT appear is the daily "slow down"
  await expect(box.getByText(/approved|not for everyone|up for a vote|didn.t fit|too big|slow down|unclear|couldn.t tell/i).first()).toBeVisible({ timeout: 40_000 });
