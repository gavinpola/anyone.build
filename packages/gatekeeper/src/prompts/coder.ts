export function coderSystemPrompt(opts: { backend?: boolean } = {}) {
  ...
  return [
    ...
    `- The wall itself is a file: src/rooms/main/canvas.ts exports \`canvas: CanvasMeta\` with background (CSS colours/gradients, no URLs), gap, radius, padding, columns (6-16), shapes (the palette handed to blocks that don't choose), tilt (max degrees), stagger, height (for free placement), flow ("organic" = dense with overlap so bodies touch; "grid" = neat rows), goo (bodies fuse where they touch, like paint), overlap (px), palette (tints for bodies), morph (outlines breathe). Asks about the wall's look, spacing, or layout edit that file. A block's own look lives in its \`block\` meta: shape (a preset "card" | "soft" | "round" | "bare" | "blob", or { radius, clip, background, color, border, shadow, padding, blob (an eight-value radius), tint (its colour on the liquid), blend ("multiply" | "screen" | "overlay" | "soft-light"), merge (fuse with neighbours) } with plain CSS values), span (1-12 columns, overrides size), tilt (-3..3), place ({ x, y, w } in world px on a bounded canvas, default 2400 × 1600, see canvas.ts size; a placed block sits exactly there; unplaced blocks are packed into free space). A target of "here X,Y" means place the new block at that point (a sensible width, 480-720); "region X,Y,W,H" means work inside that rectangle: new blocks get a place inside it, sized to fit; "Move this to x=…, y=…" / "make this W wide" = edit that block's place only. Keep every value plain CSS; never a url().`,
