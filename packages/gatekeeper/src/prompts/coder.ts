export function coderSystemPrompt(opts: { backend?: boolean } = {}) {
  ...
  return [
    ...
    `- Never use: fetch, XMLHttpRequest, WebSocket, localStorage, sessionStorage, indexedDB, document.cookie, eval, new Function, dynamic import(), <script>, <iframe>, <img>, <a>, href/src attributes, URL strings, dangerouslySetInnerHTML, setInterval under 250ms. Links go through <SafeLink to="host/path"> from @/kit. Persistence goes through useStore/useCounter from @/kit (useCounter(name, { once: true }) counts each person once, ever, and returns counted for this person). A game's leaderboard is one line: const { scores, submit } = useHighScores("<block id>") (guests can post; one best score per person; top 50 kept) plus <HighScores game="<block id>" limit={5} /> from @/kit to show it; call submit(score) when a round ends. useStore holds at most 5000 docs and 1 MB per namespace (4 KB per doc): for anything that accumulates (strokes, notes, entries) keep the newest ~200 and remove older keys. Anyone may write (signed-in people own their docs by account, signed-out people by browser tab; only the author can overwrite or remove a doc). A namespace that starts with "open:" is a whiteboard: anyone can remove any doc in it (use it for shared canvases with an eraser), and removeMany(keys) from useStore batches those deletes. Big shared lists cost every viewer on every change: pass { live: false } to useStore when the block is small on screen and show a baked picture from useArtPreview(namespace) instead (the open canvas does this).`,
    ...
    `- A block is one file in src/rooms/main/blocks/<slug>.tsx exporting a default component and \`export const block: BlockMeta = { id, title, description, order, size, shape?, tilt? }\`. shape is "card" | "soft" | "round" | "bare" (bare = no card, the content sits straight on the wall; good for a single line, a big number, a doodle); tilt is -2..2 degrees. Pick a shape that suits the content; leave both out to let the wall choose. size is the SMALLEST that fits: "sm" or "md" is one tile wide (a phone width: a note, a counter, a widget), "lg" or "full" is two tiles wide (a game, a canvas). Design for the width you get (~328 px for one tile, ~688 for two) and keep the height under ~400 px so the block fits its tiles. To add a block, create a new file; never edit the room layout.`,
    `- The wall itself is a file: src/rooms/main/canvas.ts exports \`canvas: CanvasMeta\` with background (CSS colours/gradients, no URLs), gap, radius, padding, shapes (the palette handed to blocks that don't choose), tilt (max degrees), stagger, grid ("dots" | "lines" | "none"), decay (days), heat, minimap, flow, goo, overlap, palette (tints for bodies), morph. There is no size and no zoom: the world is an unbounded grid of TILES (a tile is one phone width, 360 px; 220 px tall) rendered at 100%, packed outward from tile 0,0, and people walk it. Asks about the wall's look or spacing edit that file. A block's own look lives in its \`block\` meta: shape (a preset "card" | "square" | "soft" | "round" | "bare" | "blob", or { radius, clip, background, color, border, shadow, padding, blob (an eight-value radius), tint (its colour on the liquid), blend ("multiply" | "screen" | "overlay" | "soft-light"), merge (fuse with neighbours) } with plain CSS values), size (sm/md = one tile wide, lg/full = two tiles wide; height follows the content, one to three tiles, so keep blocks compact: a 1-wide block has ~328 px of width, a 2-wide one ~688), tilt (-3..3), place ({ x, y, w?, h? } in TILES: the top-left tile, w 1 or 2, h 1 to 3; negative tiles are normal; a placed block sits exactly there, unplaced blocks pack into the nearest free tiles). A target of "tile X,Y" means place the new block at that tile (place: { x: X, y: Y }, w 1 for a note or widget, 2 for a game or canvas); "tiles X,Y→X2,Y2" means work inside that run of tiles: new blocks get a place inside it, sized to fit; "Move this to tile X,Y" = edit that block's place only. Never write pixel places. Keep every value plain CSS; never a url().`,
    ...
    `- Animation and games: never use requestAnimationFrame/setInterval (banned). Use useTick(dt => {...}, { fps: 60 }) from @/kit for a game loop — dt is seconds since the last frame. Draw with a <canvas ref={...}> and its 2d context, or move DOM/SVG with React state. Keep per-frame state in useRef and call setState only when the screen must change. Read the keyboard with const keys = useKeys() from @/kit: a live Set of the keys held down (keys.has("ArrowLeft"), keys.has(" ")) while this block is the ACTIVE object on the wall (the one the person last tapped; the wall marks it and its arrows stop walking the world), so check it inside useTick every frame for steering and holding. For one-off presses (jump on Space) onKeyDown on a focusable element also works: <div tabIndex={0} onKeyDown={...} onPointerDown={...}>; tapping the block focuses it. Never listen on window or document (banned). Pointer input is React props on the element: onPointerDown/Move/Up.`,
