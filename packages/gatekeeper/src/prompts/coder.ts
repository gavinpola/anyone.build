export function coderSystemPrompt(opts: { backend?: boolean } = {}) {
  ...
  return [
    ...
    `- Never use: fetch, XMLHttpRequest, WebSocket, localStorage, sessionStorage, indexedDB, document.cookie, eval, new Function, dynamic import(), <script>, <iframe>, <img>, <a>, href/src attributes, URL strings, dangerouslySetInnerHTML, setInterval under 250ms. Links go through <SafeLink to="host/path"> from @/kit. Persistence goes through useStore/useCounter from @/kit. useStore holds at most 5000 docs and 1 MB per namespace (4 KB per doc): for anything that accumulates (strokes, notes, entries) keep the newest ~200 and remove older keys. Writes need sign-in, so show a small "sign in to add" state for guests (useViewer from @/kit).`,
    ...
    `- The wall itself is a file: src/rooms/main/canvas.ts exports \`canvas: CanvasMeta\` with background (CSS colours/gradients, no URLs), gap, radius, padding, columns (6-16), shapes (the palette handed to blocks that don't choose), tilt (max degrees), stagger, height (for free placement). Asks about the wall's look, spacing, or layout edit that file. A block's own look lives in its \`block\` meta: shape (a preset, or { radius, clip, background, color, border, shadow, padding } with plain CSS values), span (1-12 columns, overrides size), tilt (-3..3), place ({ x, y, w }: x and w in percent of the wall, y in px; the block sits exactly there, blocks may overlap). Keep every value plain CSS; never a url().`,
