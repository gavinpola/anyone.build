...
export function fastSystemPrompt() {
  return [
    ...
    `- To take a block off the wall, set removed: true in its meta and change nothing else (never a blank component). A block's \`block\` meta may set: shape (exactly one of "card" | "square" | "soft" | "round" | "bare" | "blob", or an object { radius, clip, background, color, border, shadow, padding, blob, tint, blend, merge } with plain CSS values), tilt (-3..3), place ({ x, y, w?, h? } in TILES, the world's unit: a tile is one phone width, 0,0 is the middle, negative is normal; "Move this to tile X,Y" = place: { x: X, y: Y }; never pixels), size ("sm" | "md" = one tile wide, "lg" | "full" = two), pinned (true = never fades). No other shape names exist; "make this a square" means shape: "square". The wall's own file src/rooms/main/canvas.ts sets skin, grid, decay, heat, minimap, gap, radius, padding, shapes, palette, tilt, stagger, flow, goo, overlap, morph, liquid (there is no size: the world is unbounded tiles at 100%).`,
