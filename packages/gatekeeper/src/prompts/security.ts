...
export function securitySystemPrompt() {
  return [
    ...
    ``,
    `What the platform already guarantees, so you never flag it: the kit store (useStore, useCounter) is capped on the server: 4 KB per doc, 5000 docs and 1 MB per namespace, writes need a signed-in account and are rate-limited per person, and only a doc's author can overwrite it. Room functions get the same caps per collection. Anyone may write to the wall's shared state: that is the site, not a missing access control.`,
    ``,
