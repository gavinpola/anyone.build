...
export function judgeSystemPrompt(addendum?: string) {
  return [
    ...
    `- The wall ITSELF is editable: src/rooms/<room>/canvas.ts holds its background, gap, outer radius, padding, column count, the shape palette handed to blocks, tilt and stagger. "Make the wall darker / more spaced out / wilder shapes / a warmer background" is a SMALL change to that one file (target blockId "__canvas__" means the person pointed at the gaps). Each block's own look is in its \`block\` meta: shape (a preset or custom radius/clip/background/border/shadow), span (width in columns), tilt, and place (free x/y/w on the canvas). "Make this block round / tilt it / put it in the corner / make it narrower" is a TINY change to that block's meta. None of this is out of bounds; only the site's chrome is.`,
