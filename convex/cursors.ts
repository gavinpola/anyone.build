...
export const move = mutation({
  args: { roomId: v.string(), sessionId: v.string(), x: v.number(), y: v.number(), hue: v.number() },
  handler: async (ctx, { roomId, sessionId, x, y, hue }) => {
    ...
    const existing = await ctx.db
      ...
      .unique();
    if (existing) await ctx.db.patch(existing._id, { x: cx, y: cy, hue: h, at: now });
    else await ctx.db.insert("cursors", { roomId, sessionId, x: cx, y: cy, hue: h, at: now });
  },
});
...
export const active = query({
  ...
  handler: async (ctx, { roomId, sessionId }) => {
    ...
    return rows
      ...
      .slice(0, MAX_CURSORS)
      .map((c) => ({ id: c.sessionId, x: c.x, y: c.y, hue: c.hue }));
  },
});
