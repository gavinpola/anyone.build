...
function useRequestsConvex(): FeedRequest[] {
  ...
  const landed = useQuery(api.requests.landed, { limit: 30 });
...
export async function submitRequest(input: { prompt: string; target: Target; handle: string; avatarUrl: string | null }): Promise<string> {
  if (!hasConvex || !convex) return mock.submit(input);
  const { blockTitle: _t, ...target } = input.target;
  const id = await convex.mutation(api.requests.submit, { prompt: input.prompt, target: { ...target, blockTitle: input.target.blockTitle } });
  return id as string;
}

export function cancelRequest(id: string) {
  if (!hasConvex || !convex) return mock.cancel(id);
  void convex.mutation(api.requests.cancel, { id: id as Id<"requests"> });
}

export function plusOneRequest(id: string) {
  if (!hasConvex || !convex) return mock.plusOne(id);
  void convex.mutation(api.requests.plusOne, { id: id as Id<"requests"> });
}
