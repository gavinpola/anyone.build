...
const modules = import.meta.glob<BlockModule>("/src/rooms/main/blocks/*.tsx", { eager: true });
...
const examples: Record<string, BlockModule> =
  import.meta.env.DEV && import.meta.env.VITE_E2E_BLOCKS === "1" ? import.meta.glob<BlockModule>("/docs/examples/blocks/*.tsx", { eager: true }) : {};

export const blocks = Object.entries({ ...examples, ...modules })
  .map(([path, mod]) => ({ path: path.slice(1), meta: mod.block, Component: mod.default }))
  .filter((b) => b.meta && b.Component)
  .sort((a, b) => a.meta.order - b.meta.order || a.meta.id.localeCompare(b.meta.id));
    ...
    <div className="grid grid-cols-12 gap-5" data-room={room.id}>
      {blocks.map(({ meta, Component, path }) => (
        <section
          ...
          data-ab-path={path}
          className={cn("frame col-span-12 flex flex-col", span[meta.size] ?? span.md)}
        >
        ...
        <div className="frame-body flex flex-1 flex-col items-center justify-center p-8 text-center">
          {empty ? (
            <>
              ...
              <h1 className="mt-3 font-display text-4xl tracking-tight sm:text-6xl">Nothing hangs here yet.</h1>
              <p className="mt-4 max-w-md text-[15px] text-ink-2">
                Hold <kbd className="rounded border border-line bg-paper-2 px-1 font-mono text-[12px]">⇧</kbd>
                <kbd className="ml-1 rounded border border-line bg-paper-2 px-1 font-mono text-[12px]">⌘</kbd>, click anywhere, say what should be here.
              </p>
            </>
          ) : (
            <p className="placard">Empty space. Point here to add something.</p>
          )}
        </div>
      </section>
    </div>
