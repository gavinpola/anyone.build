...
for (const room of readdirSync(roomsDir, { withFileTypes: true }).filter((d) => d.isDirectory())) {
  ...
  writeFileSync(path.join(out, "manifest.json"), JSON.stringify({ room: room.name, generatedAt: new Date().toISOString(), blocks }, null, 2));
  console.log(`manifest: ${room.name} (${blocks.length} blocks)`);
}
