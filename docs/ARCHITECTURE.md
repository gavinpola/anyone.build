...
npx convex env set SITE_URL https://everyones.lol
npx convex env set GITHUB_REPO anyone-build/everyones.lol
...
npx convex env set STRIPE_SECRET_KEY ... STRIPE_WEBHOOK_SECRET ... RESEND_API_KEY ...
...
Repo rulesets on `main`: require PR, require the `checks` status (add `smoke` to `REQUIRED_CHECKS` once preview deployments report), no force push, no bypass for the app. Enable "Allow squash merging" and "Automatically delete head branches". Labels: `playground`, `revert`.
