...
3. **Forbidden code.** No network calls, storage, cookies, scripts, iframes, images or links from elsewhere, URL literals, dynamic import, eval, obfuscation, or invisible unicode in room files. ESLint bans plus a regex pass (`packages/gatekeeper/src/validate`). Both run in the sandbox, in Convex before anything is committed (`packages/gatekeeper/src/lint/lint-files.js` runs the same AST rules there), and in CI on the pull request. Links only through `SafeLink` with an allowlist.
...
Blocks persist through `useStore(namespace)`: public read, anyone writes, rate-limited. A doc belongs to its author (by account when signed in, by browser tab when not) and only the author, or a maintainer, can overwrite or remove it. A namespace that starts with `open:` is a whiteboard: anyone can remove any doc in it, which is what an eraser on a shared canvas needs, and what a vandal could abuse; the limits (30 erase calls a minute, 50 keys each) slow that down, and the hourly timelapse keeps the record. The rule lives in `convex/lib/storeRules.ts` and is unit-tested. Reserved prefixes (`room:`, `counter:`) stay server-only.
