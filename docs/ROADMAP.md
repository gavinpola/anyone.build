...
- [x] The playtester's reasons feed back into the coder loop: a red PR (playtest or checks) gets one more sandbox pass told which checks failed and what the playtester saw (`playtestNote`, `run.playtestRetry`); only a second red fails the ask. Nightly sweep in place (`.github/workflows/sweep.yml`, 10:17 UTC: every block mounted and played; a failure opens or updates one issue). Still open: a `useKeys()` kit helper.
...
## Thousands of tabs (Gavin, 2026-09-04: "hundreds or thousands of people at once, what should I do prior?")
- [x] Measured on production with 30 headless viewers: every stroke on the open canvas cost each idle viewer ~15 KB (the whole stroke list re-sent), ~14 MB of fan-out per stroke at 1,000 viewers, and worse as the list grows. Fixed: the canvas is baked to a PNG once a minute (`convex/artBake.ts`, resvg, skips when unchanged; `art.latest` is a tiny query everyone can hold), and the block subscribes to live strokes only when it is on screen and at least 480 px wide, or was touched in the last 20 s (`useStore(ns, { live, limit })`, `useArtPreview`). Viewers at the overview cost nothing per stroke.
...
- [x] Re-measured a day later (30 idle viewers, one drawing): ≈3 KB per stroke per viewer, down from 14.7 KB; the remainder is cursor traffic (throttle 120 → 200 ms).
- [ ] NEXT: a stress drill of 20 asks in 5 minutes (needs a go: it puts 20 changes on the wall); a GitHub merge queue so a burst of green PRs is one deploy.
