...
- [ ] NEXT: an in-house "how it's going" panel on /admin from Convex data (asks per hour, verdict mix, builds, cost, erases) so the numbers are visible without a third party; Vercel Web Analytics is a one-click toggle in the Vercel dashboard if traffic curves are wanted without PostHog.
...
- [ ] NEXT: a stress drill against production (20 asks in 5 minutes as guests) once the snapshot and nudges have run for a day; Octokit throttling for merge bursts; a "how it's going" panel on /admin from `stats.outcomes`.
