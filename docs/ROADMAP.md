...
- [x] The playtester's reasons feed back into the coder loop: a red PR (playtest or checks) gets one more sandbox pass told which checks failed and what the playtester saw (`playtestNote`, `run.playtestRetry`); only a second red fails the ask. Still open: a nightly playtest sweep of the whole wall so regressions from one block's change to another are caught; a `useKeys()` kit helper.
